...
# Splitting our data
data_splitter = DataSplitter("diagonals_data.csv", [8, 1, 1])
training_data, validation_data, testing_data = data_splitter.split()

# Setting up the neural network
learning_rate = 0.01
network = Network(2, 2, [4, 4],
                  learning_rate=learning_rate, num_classes=4, adaptive=True,
                  he_weights=True)
...
trainer.run()
trainer.generate_loss_plot(title=f'diagonals_lr_{learning_rate}')
trainer.generate_scatter(title=f'diagonals_lr_{learning_rate}')
validator.generate_scatter(title=f'diagonals_lr_{learning_rate}')

# Running the tester and generating some plots
tester.run()
tester.generate_confusion()
tester.generate_scatter(title=f'diagonals_lr_{learning_rate}')
